...
if [ "$CORE_ONLY" -eq 0 ] && [ -f webui/package.json ]; then
  if [ ! -f webui/dist/index.html ] || find webui/src webui/index.html -type f -newer webui/dist/index.html -print -quit 2>/dev/null | grep -q .; then
    if [ -d webui/node_modules ] && command -v npm >/dev/null; then
      echo "building Thunder WebUI"
      npm --prefix webui run build
    else
      echo "WARN: WebUI is not built; run: npm --prefix webui install && npm --prefix webui run build"
    fi
  fi
fi
...
if [ "$CORE_ONLY" -eq 1 ]; then
  exec node daemon/host/src/entry.mjs --profile thunderd-core
fi
# 同一 launcher 选择全量 profile；Web API 仍由独立子进程运行。
exec node daemon/host/src/entry.mjs --profile thunderd
