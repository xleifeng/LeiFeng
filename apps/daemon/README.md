...
一条命令：`bash apps/daemon/run.sh`（systemd 前台模式也可用 `bash apps/daemon/run.sh --systemd`）会启动 daemon core 与独立 Web API；对外 JSON-RPC 监听 `127.0.0.1:16800`（`THUNDERD_PORT` 可改）。完整分层、依赖方向与独立启动方式见仓库根目录 `ARCHITECTURE.md`。
...
# 裸 core（不含 Web API）
node apps/daemon/host/src/entry.mjs --profile thunderd-core
# 查看脱敏后的最终装配配置（不启动、不取锁）
node apps/daemon/host/src/entry.mjs --profile thunderd --dump-config
# 用 JSON 覆写插件配置
node apps/daemon/host/src/entry.mjs --profile thunderd --config ./my.json
```

`--config` 文件只接受 `{"plugins": [{"id": "<插件ID>", "enabled"?: boolean, "config"?: object}]}`；叠加顺序为 profile 默认 → profile patch → 用户 patch，重复 provider、缺依赖、未知字段会在启动前报错。secret 字段与磁力 URI 在 dump 中一律脱敏。桥的 `bridge-host` profile 见 `apps/bridge/README.md`。
...
```bash
node apps/daemon/host/bin/migrate-v2.js --runtime "$THUNDERD_RUNTIME_DIR" --dry-run --report "$THUNDERD_RUNTIME_DIR/data/migration-manifest.json"
node apps/daemon/host/bin/migrate-v2.js --runtime "$THUNDERD_RUNTIME_DIR" --apply --report "$THUNDERD_RUNTIME_DIR/data/migration-manifest.json"
node apps/daemon/host/bin/migrate-v2.js --runtime "$THUNDERD_RUNTIME_DIR" --verify --report "$THUNDERD_RUNTIME_DIR/data/migration-manifest.json"
```

`--apply` 不覆盖已有 V2 文件；首次迁移时先创建精确回滚备份、再原子写入 `data/tasks.json`，不会删除 `registry.json`。确认 hash、任务数、回滚备份和真实四协议/破坏性操作验收均有证据后，才允许物理删除兼容层。`apps/daemon/integration/migration-dry-run.js` 仍可作为无写入快速预检。
...
- `apps/daemon/host/src/stack.js` 只负责监管：Web API 异常退出时单独重启，不停止 daemon core 或进行中的 P2SP/VIP 下载；daemon core 退出时才收拢整个栈。
...
daemon unit 使用 `run.sh --systemd --core-only` 前台运行，Web API unit 使用 `apps/web-api/run.sh --systemd`。两个进程分别处理 `SIGTERM` 并由 systemd 独立重启。若不以 root 运行，应在两个 unit 中使用同一 `User=<service-user>`，并确保该用户拥有 runtime socket、Wine prefix、下载目录及完整 `thunder_x/program/SDK/` 的访问权限。
